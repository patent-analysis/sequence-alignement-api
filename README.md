...
```bash
sequence-alignment-api$ sam build
```
...
```bash
sequence-alignment-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sequence-alignment-api$ sam local start-api
sequence-alignment-api$ curl http://localhost:3000/
```
...
```bash
sequence-alignment-api$ sam logs -n HelloWorldFunction --stack-name sequence-alignment-api --tail
```
...
```bash
sequence-alignment-api$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name sequence-alignment-api
```
